...
async function isAdmin(authHeader: string) {
  ...
  if (userError || !userData?.user) return { ok: false, message: "Unauthorized", userId: null };
  ...
  if (profileError) return { ok: false, message: "Forbidden", userId: userData.user.id };
  if (profile?.role !== "admin") return { ok: false, message: "Forbidden", userId: userData.user.id };
...
Deno.serve(async (req) => {
  ...
  if (adminCheck.userId === userId) {
    return new Response(JSON.stringify({ error: "Admin tidak bisa menghapus akun sendiri" }), {
      status: 400,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
