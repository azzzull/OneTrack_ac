...
const AttendanceLog = () => {
  ...
  const { user } = useAuth();
  const { getAdminAttendanceLog, loading, getTodayAttendance } = useAttendance();
  ...
  // Map modal
  const [mapModal, setMapModal] = useState({ isOpen: false, data: null, type: null, technicianName: '' });
  ...
  useEffect(() => {
    applyFilters();
  }, [attendanceData, filterTechnician, filterDateFrom, filterDateTo, filterStatus]);
  ...
  const applyFilters = () => {
    ...
    if (filterTechnician) {
      filtered = filtered.filter((item) => item.technician_id === filterTechnician);
    }

    if (filterDateFrom) {
      filtered = filtered.filter((item) => item.attendance_date >= filterDateFrom);
    }

    if (filterDateTo) {
      filtered = filtered.filter((item) => item.attendance_date <= filterDateTo);
    }

    if (filterStatus === 'check_in_only') {
      filtered = filtered.filter((item) => item.check_in_time && !item.check_out_time);
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredData.map((record) => [
      ...
      record.working_hours_minutes ? formatWorkingHours(record.working_hours_minutes) : '-',
    ]);
  ...
  return (
    <div className="min-h-screen bg-sky-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <label className="block">
                ...
                >
                  ...
                  <option value="check_in_only">Hanya Masuk</option>
                  <option value="checked_in_and_out">Masuk & Pulang</option>
                </select>
              </label>
            </div>
          </div>
          ...
          {filteredData.length > 0 && (
            <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="rounded-xl bg-blue-50 border border-blue-200 p-4">
                <p className="text-sm text-blue-600 font-medium">Total Record</p>
                <p className="text-2xl font-bold text-blue-900 mt-2">{filteredData.length}</p>
              </div>

              <div className="rounded-xl bg-green-50 border border-green-200 p-4">
                <p className="text-sm text-green-600 font-medium">Hari Kerja Lengkap</p>
                <p className="text-2xl font-bold text-green-900 mt-2">
                  {filteredData.filter((r) => r.working_hours_minutes).length}
                </p>
              </div>

              <div className="rounded-xl bg-amber-50 border border-amber-200 p-4">
                <p className="text-sm text-amber-600 font-medium">Belum Check-Out</p>
                <p className="text-2xl font-bold text-amber-900 mt-2">
                  {filteredData.filter((r) => !r.check_out_time).length}
                </p>
              </div>

              <div className="rounded-xl bg-indigo-50 border border-indigo-200 p-4">
                <p className="text-sm text-indigo-600 font-medium">Total Jam Kerja</p>
      ...
      <AttendanceMapModal
        ...
        onClose={() => setMapModal({ isOpen: false, data: null, type: null, technicianName: '' })}
        data={mapModal.data ? { ...mapModal.data, technician_name: mapModal.technicianName } : null}
        type={mapModal.type}
      />
    </div>
  );
