...
const AttendanceMap = ({ technicianId }) => {
  ...
  const buildMarkers = (data) => {
    ...
    if (data.check_in_latitude && data.check_in_longitude) {
      markerList.push({
        ...
        label: 'Lokasi Masuk',
        info: {
          waktu: new Date(data.check_in_time).toLocaleTimeString('id-ID', {
            hour: '2-digit',
            minute: '2-digit',
            second: '2-digit',
          }),
          ...
          akurasi:
            data.check_in_accuracy_meters && data.check_in_accuracy_meters > 0
    ...
    if (data.check_out_latitude && data.check_out_longitude) {
      markerList.push({
        ...
        label: 'Lokasi Pulang',
        info: {
          waktu: new Date(data.check_out_time).toLocaleTimeString('id-ID', {
            hour: '2-digit',
            minute: '2-digit',
            second: '2-digit',
          }),
          ...
          akurasi:
            data.check_out_accuracy_meters && data.check_out_accuracy_meters > 0
  ...
  return (
    <div className="mb-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-3">Lokasi Hari Ini</h3>
      <LeafletMap
        markers={markers}
        center={center}
        zoom={zoom}
      ...
      <p className="text-xs text-gray-500 mt-2">
        {markers.length} lokasi tercatat ({markers.map(m => m.label).join(', ')})
      </p>
    </div>
  );
};
